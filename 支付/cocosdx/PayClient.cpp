...
PayClient::PayClient(void):m_socket( SOCKET_ERROR ),m_log("pay_client.log")
{
	m_log.M_Log( "PayClient Creat!" );
}
...
int PayClient::Pay(double price, string thing, string description, string &orderID )
{
	...
	m_log.M_Log( "Recv OrderID recv_buffer:%s",recv_buffer );
	orderID = recv_buffer;
	return 1;
}
